...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                ###################################################################
                # prompt = [prop['type'] for prop in properties]#最开始训练的时候使用 但是推理的时候报错，因此修改这个和nnunettrainer中的一致（额外加了一个判断）
                try:
                ...
                # print(prompt)#['brea', 'brea']
                #推理的时候用到 因为不是一个batch的格式['brea', 'brea']->[['brea', 'brea']]  推理的时候 patch的batch是1
                ...
                prediction = self.predict_logits_from_preprocessed_data(data,prompt).cpu()
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor,prompt: torch.Tensor) -> torch.Tensor:
        """
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor,prompt: torch.Tensor) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        prediction = self.network(x,prompt)
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction += torch.flip(self.network(torch.flip(x, (*axes,)),prompt), (*axes,))
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                prediction = self._internal_maybe_mirror_and_predict(workon,prompt)[0].to(results_device)
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor,prompt: torch.Tensor) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers,prompt,
                                                                                               self.perform_everything_on_device)
                    except RuntimeError:
                        print(
                            'Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers, prompt,False)
                else:
                    predicted_logits = self._internal_predict_sliding_window_return_logits(data, slicers,prompt,
                                                                                           self.perform_everything_on_device)
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset226_uni_seg/STUNetTrainer_small_VIT___nnUNetPlans__3d_fullres'),#Dataset221_AutoPETII_2023/nnUNetTrainer__nnUNetPlans__3d_fullres
        # use_folds=(0,1,2,3,4),
        use_folds=(0,),
    ...
    predictor.predict_from_files(join(nnUNet_raw, '/projects/nki-breast-mri/snapshot/a_PETCT/firstDiagnose/beforeSurgery_nnunet/imagesTs_miccai'),
                                 join(nnUNet_raw, '/projects/nki-breast-mri/snapshot/a_PETCT/firstDiagnose/beforeSurgery_nnunet/imagesTs_predlowres_prompt_miccai_'),
